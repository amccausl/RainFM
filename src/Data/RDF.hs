...
--  empty = mkRdf [] Nothing (PrefixMappings Map.Empty)
  ...
  mkRdf :: Triples -> Maybe BaseUrl -> PrefixMappings -> gr
  ...
  query         :: gr -> Maybe Subject -> Maybe Predicate -> Maybe Object -> Triples
...
class RdfParser p where
  ...
  -- the resultant graph.
  parseString :: forall gr. (Rdf gr) => p -> ByteString -> (Either ParseFailure gr)
  ...
  parseFile   :: forall gr. (Rdf gr) => p -> String     -> IO (Either ParseFailure gr)
  ...
  parseURL    :: forall gr. (Rdf gr) => p -> String     -> IO (Either ParseFailure gr)
...
class RdfSerializer s where
  ...
  -- the first argument.
  hWriteG     :: forall gr. (Rdf gr) => s -> Handle -> gr -> IO ()

  -- |Write the graph to stdout; equivalent to @'hWriteG' stdout@.
  writeG      :: forall gr. (Rdf gr) => s -> gr -> IO ()
  ...
  hWriteH     :: forall gr. (Rdf gr) => s -> Handle -> gr -> IO ()

  -- |Write header information to stdout; equivalent to @'hWriteH' stdout@.
  writeH      :: forall gr. (Rdf gr) => s -> gr -> IO ()
...
fromEither :: Rdf gr => Either ParseFailure gr -> gr
fromEither res =
  case res of
